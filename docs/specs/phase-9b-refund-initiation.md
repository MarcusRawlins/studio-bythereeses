...
## Rev 4 (Fable money-math review) — changelog

Rev 4 fixes one BLOCKER and two MINORs from an adversarial Fable money-math review of rev 3. Each fix was verified against the real schema/code before landing; **all rev-2 rails and rev-3 policy are preserved intact.** This is money-moving code — the BLOCKER fix provably closes an over-refund path.

| Finding | Severity | What changed |
| --- | --- | --- |
| **F1** — §3.1 cap `max(webhookRefundedCents, ΣlocalInFlight)` **under-counts** in a mixed external-refund + 9b sequence, over-refunding by up to `clientFeeCents` per payment (violates P11 "fees passed to client"). `max()` is only correct when both terms measure the **same** refunds; when an external (dashboard) refund and a 9b refund measure **disjoint** refunds, `max` takes the larger single term and misses the sum. The rev-2 gross ceiling was backstopped by Stripe's own gross cap; rev-3's tighter service ceiling opened a gap of exactly `clientFeeCents` in the webhook-lag window. | **BLOCKER** | §3.1 redefines the already-refunded term as a **dedupe-union**: `alreadyRefundedServiceCents = max( webhookRefundedCents, Σlocal(submitting,succeeded) + Σexternal )`, where `Σexternal` = the sum of THIS payment's `payment_refunds` rows whose `stripe_refund_id` matches **no** `refund_initiations.stripe_refund_id` (i.e. refunds NOT initiated by 9b). In pure-9b flows `Σexternal = 0` and the formula is identical to rev 3. The provably-false "no unsafe divergence exists" bullet is **replaced** with the corrected analysis + honest residual. New test §9.22 exercises the exact interleave (external partial + 9b partial + lag → next 9b refund blocked, maxRefundable 0, zero Stripe calls). Must-not-get-wrong item #2 now names the dedupe-union (never plain `max(webhook, local)`). |
| **F2** (MINOR) — `isRetainerPaymentLabel` is module-PRIVATE (`function`, not `export function`, sales.ts:102), so task 3 would be forced to **copy** the predicate that defines `retainer_paid` rather than reuse it, violating "reuse, don't re-invent." | MINOR | §3.8 / task 3 gains an explicit build step: **EXPORT `isRetainerPaymentLabel`** (or extract to a shared module) so the retainer block reuses the exact predicate that drives `shouldAdvanceToRetainerPaid`, not a divergent copy. |
| **F3** (MINOR) — the earliest-payment arm of the retainer predicate is a **time-of-read** property with two silent edges. | MINOR | §3.8 states both: (a) adding a payment row with an **earlier** `dueDate` later re-classifies "earliest," so an unconventionally-labeled retainer can silently become refundable (or a former non-retainer become blocked); (b) a **NULL-`dueDate`** retainer sorts AFTER dated payments (NULLS LAST), so the earliest arm can miss it, leaving only the label arm. Both are within Risk 6's spirit — flagged for Tyler to confirm the real shape at the deploy gate. |

Also landed (verified build note): legacy **pre-9a `paid` rows can have `paidAmountCents = 0`** (the service basis was only backfilled from 9a onward) → the service ceiling caps `maxRefundableCents` to 0 → those rows **over-block** (the safe direction). The builder must **not** "fix" this by reverting the ceiling to gross — over-blocking is correct for a money-out gate; a stale-basis row is simply not 9b-refundable until its basis is real.
...
webhookRefundedCents       = payment.refundedAmountCents           // 9a webhook set-to-authoritative (cumulative amount_refunded on the charge)
localInFlightRefundedCents = Σ refund_initiations.amountCents       // THIS payment's OWN 9b refunds
                              WHERE status IN ('submitting','succeeded')
externalRefundedCents      = Σ payment_refunds.amountCents          // THIS payment's refunds NOT initiated by 9b
                              WHERE invoice_payment_id = payment.id
                                AND stripe_refund_id NOT IN (
                                      SELECT stripe_refund_id FROM refund_initiations
                                      WHERE invoice_payment_id = payment.id
                                        AND stripe_refund_id IS NOT NULL)
alreadyRefundedServiceCents = max( webhookRefundedCents,                                  // DEDUPE-UNION (F1) — never plain max(webhook, local)
...
**Why the dedupe-union, not `max(webhook, local)` (F1 — the money-critical correction).** `max()` between two terms is only correct when both terms measure the **SAME** refunds; then the larger one is authoritative and the smaller is a lagging under-report. But `webhookRefundedCents` and `localInFlightRefundedCents` can measure **DISJOINT** refunds — a Stripe **dashboard/external** refund is counted by the webhook (it lands in `refunded_amount_cents` via `charge.refunded`) but has **no** `refund_initiations` row, while a fresh 9b refund is counted locally but may not have hit the webhook yet. When the two sets are disjoint, `max` returns the larger single term and **misses the other entirely** — under-counting the true total refunded, which over-permits the next refund. The dedupe-union fixes this by adding the 9b-local total to the **external-only** total (`Σexternal`, the `payment_refunds` rows with no matching 9b initiation), and taking `max` of THAT against the webhook figure (which remains the authoritative floor once every refund has been recorded). `Σexternal` deliberately excludes any `payment_refunds` row that a 9b initiation already accounts for (matched on `stripe_refund_id`), so a 9b refund is **never double-counted** across the local and external terms.

Rev 4 adopts this **dedupe-union (option a)** because it is provably correct **and** preserves the partial-refund UX (a payment that took an external partial refund is still 9b-refundable up to its remaining service balance). The **stricter alternative (option b), noted but not adopted:** refuse 9b entirely on any payment that has an external (non-9b) `payment_refunds` row — i.e. treat the presence of a dashboard refund as "hands off, reconcile manually." Option (b) is even simpler to prove safe (no arithmetic union at all) but sacrifices partial-refund UX after any external touch. If Tyler prefers maximum conservatism over UX, option (b) is the fallback; Rev 4's default is (a).

**Failing walk this closes** (client-pays-fee: `paidAmountCents = 20000` service, `grossCollectedCents = 20590`, `clientFeeCents = 590`, all real per stripe-checkout.ts:374–391; the 9a clamp at stripe-refunds.ts:224 admits any partial webhook value up to gross):
1. External Stripe-**dashboard** partial refund of $50 → 9a sets `webhookRefundedCents (W) = 5000`. There is no `refund_initiations` row for it, but there **is** a `payment_refunds` child row (`stripe_refund_id` present, `invoice_payment_id` = this payment) → `Σexternal = 5000`.
2. 9b refund: cap = `max(5000, 0 + 5000) = 5000` → `maxRefundable = 15000`. Admin refunds 15000 → succeeds; the 9b initiation row stores its `re_…` `stripe_refund_id`. Stripe cumulative = 20000. `Σlocal = 15000`.
3. **During webhook lag** (`W` still 5000, `charge.refunded` for the 15000 not yet landed): cap = `max(5000, 15000 + 5000) = 20000` → **`maxRefundable = 0`** → any further 9b request is **blocked before any Stripe call.** (Under the old `max(5000, 15000) = 15000` this returned `maxRefundable = 5000`; Stripe's remaining refundable was `20590 − 20000 = 590`, so a ≤ $5.90 request passed every local gate AND Stripe, returning gross = 20590 > 20000 service → the client got the card fee back. P11 violated by exactly `clientFeeCents`. The dedupe-union zeroes it.)
...
**Build note — legacy pre-9a `paid` rows (verified, safe direction).** `paidAmountCents` (the service basis) was only populated by the settle path from 9a onward; a legacy `paid` row from before that backfill can have `paidAmountCents = 0`. The service ceiling then caps `maxRefundableCents` to 0, so those rows **over-block** (no 9b refund until the basis is real) — the **safe** direction for a money-out gate. The builder must **NOT** "fix" this by reverting the ceiling to `grossCollectedCents`; over-blocking a stale-basis row is correct, and reverting to gross would re-open the fee-refund path (F1/P11). A legacy row that genuinely needs refunding is handled out-of-band (Stripe dashboard), not by loosening the ceiling.
...
- **Why the local + external union at all** (critical, do not omit): if the 9a webhook is lagging or (mis-)configured, `refundedAmountCents` may still be 0 right after a successful refund. Counting this payment's own non-failed `refund_initiations` rows (`Σlocal`) means a second refund is capped correctly **even before the webhook lands**, closing the double-refund window that would otherwise exist during webhook lag. Adding `Σexternal` (the dashboard/external refunds the webhook already recorded but 9b never initiated) closes the disjoint-set gap in F1. `payment_refunds`/`refunded_amount_cents` remain webhook-owned; this is a *read-side* union for the cap only, not a canonical write.
- **Corrected divergence analysis (F1 — replaces the rev-3 "no unsafe divergence exists" bullet, which was provably false).** The rev-3 spec claimed an external (non-9b) refund could only drive the cap *too conservative* and "no unsafe divergence exists." **That was wrong.** With `alreadyRefunded = max(webhook, Σlocal)` and a **service** ceiling, an external refund counted only by `webhook` and a 9b refund counted only by `Σlocal` are disjoint; `max` takes the larger and drops the smaller, **under-counting** the true total — an *over*-permit of up to `clientFeeCents` per payment (the failing walk above). The dedupe-union fixes exactly this by summing the disjoint local + external totals before the `max`. **Residual, stated honestly:** an external refund whose webhook has **not landed at all yet** is locally invisible — it is neither in `webhookRefundedCents` nor in `payment_refunds` (so not in `Σexternal`) until the webhook arrives. Exposure in that narrow no-webhook window stays **bounded by `clientFeeCents`** and only exists between an external refund and its own webhook; it is surfaced by the ~1h stuck/reconcile tripwire (§4.4) and the read-time over-cap / divergence flag (9a §1.5). On `studio_absorbs` (`clientFeeCents = 0`) the exposure is **zero**. Direction of every *other* case remains the safe one (over-block, never over-permit).
- **Basis consistency of the union (flagged for Tyler).** All three terms are treated as **cents-refunded-on-the-service-basis** quantities. `Σlocal` is exactly that by construction — 9b only ever sends service-portion amounts. `webhookRefundedCents` and `Σexternal` reflect Stripe's *actual* refund amounts, which for an external gross refund could include the fee portion; if so they **over-count** (larger already-refunded → smaller `maxRefundable`), the **safe** direction. So a fee-inclusive external refund tightens the cap, it never loosens it.

**Must-not-get-wrong (the money-math invariants — the builder verifies each against §9 tests before flipping the flag).**
1. **Ceiling is the SERVICE portion (`paidAmountCents`), never gross** (P11). Reverting to `grossCollectedCents` re-opens the fee-refund path. (§9.3, §9.18, §9.19.)
2. **The already-refunded term is the DEDUPE-UNION `max(webhook, Σlocal + Σexternal)` — NEVER plain `max(webhook, Σlocal)`** (F1). Plain `max` under-counts a disjoint external-refund + 9b sequence and over-refunds by up to `clientFeeCents`. `Σexternal` must exclude any `payment_refunds` row already matched to a 9b `refund_initiations.stripe_refund_id`, so a 9b refund is never double-counted. (§9.4, §9.22.)
3. **The amount sent to Stripe is the ROW's persisted `amount_cents` (pinned in the `submitting` UPDATE), not the prepare-time prefill** (M3), so the row, the Σ-cap, and the Stripe request are byte-identical. (§9.15.)
4. **A `submitting` row is NEVER blind-retried** — recovery is reconcile-against-Stripe, because the idempotency key expires ~24h (M1). (§9.6, §9.14.)
5. **All caps are re-computed against the FRESH ledger at execute**, not the draft-time value — a refund that landed via webhook between prepare and execute shrinks the cap. (§9.4.)
...
- **Build step (F2 — `isRetainerPaymentLabel` is currently module-PRIVATE).** Verified: at sales.ts:102 the predicate is `function isRetainerPaymentLabel(label)` returning `/\b(retainer|deposit)\b/i.test(label ?? "")`, and it drives `shouldAdvanceToRetainerPaid` — but it is **not exported.** So task 3 must first **EXPORT `isRetainerPaymentLabel`** from `sales.ts` (or extract it to a shared module, e.g. `src/lib/payment-labels.ts`, imported by both `sales.ts` and `stripe-refund-initiation.ts`). The retainer block must call the **exact same predicate** that defines `retainer_paid`, not a copy of the regex — a divergent copy is precisely the "re-invent" the spec forbids and would let the two meanings of "the retainer" drift. This is an explicit build step in task 3 (§10) and §3.8, not an implementation detail.
...
- **The earliest-payment arm is a TIME-OF-READ property — two silent edges (F3), both within Risk 6's spirit, for Tyler to confirm the real shape at the deploy gate.** (a) **Re-classification on later inserts:** "earliest" is evaluated over the invoice's payments *at execute time*, so adding a payment row with an **earlier `dueDate`** later shifts which row is "earliest" — an unconventionally-labeled retainer that *was* the earliest can silently become **refundable** once an earlier-dated row is added (and, symmetrically, a former non-retainer can become blocked). (b) **NULL-`dueDate` retainers sort LAST:** the order is `dueDate NULLS LAST`, so a retainer with a **NULL `dueDate`** sorts *after* every dated payment and the earliest arm **misses it** — leaving only the label arm to catch it. Neither edge over-permits silently in the common case, but both mean "earliest" is not a stable identity; confirm with Tyler that retainers are the earliest-dated (non-NULL `dueDate`) row, or rely on the label.
...
4. **Ledger re-check uses the DEDUPE-UNION `max(webhook, Σlocal + Σexternal)` (F1), not plain `max(webhook, Σlocal)`** — with `refundedAmountCents = 0` but a prior `succeeded`/`submitting` `refund_initiations` row summing to the full **service** amount, a second refund is capped/rejected → **zero** Stripe calls (proves the webhook-lag double-refund window is closed). Also assert the cap is re-computed against the **fresh** ledger at execute (a webhook landing between prepare and execute shrinks it). See §9.22 for the mixed external + 9b interleave the plain `max` got wrong.
...
22. **External-refund + 9b interleave — dedupe-union blocks the over-refund (F1 regression, money-critical).** Set up the exact failing walk: a client-pays-fee payment (`paidAmountCents = 20000`, `grossCollectedCents = 20590`, `clientFeeCents = 590`). (a) Seed an **external** (non-9b) refund of `5000`: a `payment_refunds` row with `invoice_payment_id` = this payment, a `stripe_refund_id` matching **no** `refund_initiations` row, and `refundedAmountCents (W) = 5000`. Assert `maxRefundableCents = 15000` (`max(5000, 0 + 5000)`). (b) Execute a 9b refund of `15000` → succeeds, one `POST /v1/refunds`, the initiation row stores its `re_…` `stripe_refund_id`. (c) **Simulate webhook lag**: leave `W = 5000` (the `charge.refunded` for the 15000 not yet applied). Now assert a **further** 9b refund request (even for `1`…`590`) is **REJECTED** with `maxRefundableCents = 0` (`max(5000, 15000 + 5000) = 20000`) and makes **ZERO** Stripe calls. Assert the same request under the OLD plain `max(5000, 15000) = 15000` would have returned `maxRefundable = 5000` and (this is the bug) allowed a refund that returns gross > service — i.e. this test fails against the plain-`max` implementation and passes only against the dedupe-union. Also assert `Σexternal` does **not** double-count the 9b refund (its `payment_refunds` row, once it lands, matches the initiation's `stripe_refund_id` and is excluded), and that on a `studio_absorbs` payment (`clientFeeCents = 0`) the analogous sequence leaves zero exposure.
...
| 3 | `src/lib/stripe-refund-initiation.ts`: `initiateInvoicePaymentRefund` — eligibility + **SERVICE-basis** ledger re-check (§3.1, **dedupe-union `max(webhook, Σlocal + Σexternal)`** on `paidAmountCents`, F1 — never plain `max(webhook, Σlocal)`), **retainer hard-block (§3.8, reuse the EXPORTED `isRetainerPaymentLabel` — first export it / extract to shared module, F2; do NOT copy the regex)**, **service-not-rendered affirmation + required reason (§3.9)**, `refund_initiations` lifecycle, Stripe call with Idempotency-Key, fail-closed secret, no-log, activity logs, `actorType!=="admin"` throw. | **L** | **High (moves real money — correctness of cap/idempotency/eligibility/retainer-block is load-bearing)** |
