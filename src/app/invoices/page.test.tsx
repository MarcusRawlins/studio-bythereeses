...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "reese-invoices-page-"));
...
async function main() {
  ...
  database.prepare(`
    INSERT INTO invoices (
      ...
      'invoice-1', 'project-1', 'INV-CLIENT-FEE', 'partially_paid', 900000, 300000,
      'client_pays', 26130, ?, ?
    )
  `).run(now, now);
  database.prepare(`
    INSERT INTO invoices (
      ...
      'invoice-orphan', 'project-orphan', 'INV-NEEDS-CLIENT', 'sent', 200000, 0,
      'studio_absorbs', 0, ?, ?
    )
  `).run(now, now);
  database.prepare(`
  ...
  const markup = renderToStaticMarkup(await InvoicesPage({
    searchParams: Promise.resolve({}),
  }));

  assert.match(markup, /INV-CLIENT-FEE/);
  assert.match(markup, /INV-NEEDS-CLIENT/);
  assert.match(markup, /Needs primary client/);
  assert.match(markup, /Outstanding/);
  assert.match(markup, /\$8,174/);
  assert.doesNotMatch(markup, /\$6,000/);

  console.log("invoices page tests passed");
}
