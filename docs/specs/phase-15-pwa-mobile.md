...
- **Any request carrying `Cookie` / `Authorization`** — belt-and-suspenders; the static allowlist paths are cookieless anyway, so a credentialed request to an allowlisted path is treated as a cache-bypass.
- **Any response that is not `status === 200` + `response.type === "basic"`**, or that carries `Set-Cookie`, `Cache-Control: private`, or `Cache-Control: no-store` — never store it (guards against a static path unexpectedly returning a personalized/error response).
...
- **Never cacheable → `false` (the data-leak asserts — these are load-bearing):**
  ...
  - a GET to an allowlisted path carrying a `Cookie` header — **false** (bypass).
- Assert the response-side guard: a `200` `basic` response with `Set-Cookie` or `Cache-Control: private`/`no-store` is **not** stored.
