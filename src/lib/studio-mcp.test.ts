...
async function main() {
  ...
  assert.equal(invoiceCall.result.isError, false);
  assert.equal(invoiceCall.result.structuredContent.invoice.invoiceNumber, "INV-MCP");
  assert.equal(invoiceCall.result.structuredContent.invoice.totalCents, 1075000);
  assert.equal(invoiceCall.result.structuredContent.invoice.cardFeePolicy, "client_pays");
  assert.equal(invoiceCall.result.structuredContent.invoice.clientPayableCents, 1106205);
  assert.equal(invoiceCall.result.structuredContent.invoice.stripePaymentLink, "https://pay.stripe.com/test_mcp_invoice");
  assert.equal(invoiceCall.result.structuredContent.invoice.sourceType, "project_source");
  assert.equal(invoiceCall.result.structuredContent.invoice.sourceId, sourceCall.result.structuredContent.projectSource.id);
  ...
  const invoiceUpdateCall = await handleStudioMcpMessage({
    ...
    params: {
      ...
      arguments: {
        projectId: "project-1",
        invoiceId: invoiceCall.result.structuredContent.invoice.invoiceId,
        status: "sent",
        totalCents: 1200000,
        retainerPercent: 25,
        installmentCount: 2,
  ...
  assert.equal(invoiceUpdateCall.result.isError, false);
  assert.equal(invoiceUpdateCall.result.structuredContent.invoice.invoiceId, invoiceCall.result.structuredContent.invoice.invoiceId);
  assert.equal(invoiceUpdateCall.result.structuredContent.invoice.status, "sent");
  assert.equal(invoiceUpdateCall.result.structuredContent.invoice.totalCents, 1200000);
  assert.equal(invoiceUpdateCall.result.structuredContent.invoice.clientPayableCents, 1234830);
  assert.deepEqual(database.prepare(`
    SELECT label, amount_cents, due_date
    FROM invoice_payments
    WHERE invoice_id = ?
    ORDER BY amount_cents, label
  `).all(invoiceCall.result.structuredContent.invoice.invoiceId), [
    { label: "Retainer", amount_cents: 300000, due_date: "2026-06-15" },
    { label: "Payment 1", amount_cents: 450000, due_date: "2026-07-15" },
    { label: "Payment 2", amount_cents: 450000, due_date: "2026-08-14" },
  ]);
  ...
  assert.equal(paymentCall.result.isError, false);
  assert.equal(paymentCall.result.structuredContent.payment.invoiceId, "invoice-1");
  assert.equal(paymentCall.result.structuredContent.payment.paymentId, "payment-2");
  assert.equal(paymentCall.result.structuredContent.payment.status, "paid");
  assert.equal(paymentCall.result.structuredContent.payment.clientFeeCents, 2205);
  assert.equal(paymentCall.result.structuredContent.payment.stripeCheckoutUrl, "https://checkout.stripe.com/c/pay/cs_test_mcp_invoice_final");
  assert.equal(paymentCall.result.structuredContent.payment.stripeCheckoutSessionId, "cs_test_mcp_invoice_final");
  assert.equal(paymentCall.result.structuredContent.payment.sourceType, "project_source");
  assert.equal(paymentCall.result.structuredContent.payment.sourceId, sourceCall.result.structuredContent.projectSource.id);
  ...
  assert.equal(paymentUpdateCall.result.isError, false);
  assert.equal(paymentUpdateCall.result.structuredContent.payment.invoiceId, "invoice-1");
  assert.equal(paymentUpdateCall.result.structuredContent.payment.paymentId, "payment-2");
  assert.equal(paymentUpdateCall.result.structuredContent.payment.paidAmountCents, 74000);
  assert.equal(paymentUpdateCall.result.structuredContent.payment.externalPaymentId, "pi_mcp_final_123_corrected");
  ...
  assert.equal(schedulerPaymentCall.result.isError, false);
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.bookingId, "booking-1");
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.projectId, "project-1");
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.status, "paid");
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.clientFeeCents, 755);
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.paymentSourceType, "project_source");
  assert.equal(schedulerPaymentCall.result.structuredContent.payment.paymentSourceId, sourceCall.result.structuredContent.projectSource.id);
  ...
  assert.equal(schedulerPaymentUpdateCall.result.isError, false);
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.bookingId, "booking-1");
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.status, "paid");
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.paidAmountCents, 24000);
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.externalPaymentId, "pi_mcp_scheduler_123_corrected");
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.paymentSourceType, "project_source");
  assert.equal(schedulerPaymentUpdateCall.result.structuredContent.payment.paymentSourceId, sourceCall.result.structuredContent.projectSource.id);
  ...
  assert.deepEqual(contextCall.result.structuredContent.projectContext.schedulerBookings, [
    {
      ...
      paidAmountCents: 24000,
      clientFeeCents: 726,
      processingFeeCents: 726,
      grossCollectedCents: 24726,
      netDepositCents: 24000,
      paymentMethod: "stripe",
      paidAt: "2026-05-29T13:30:00.000Z",
      externalPaymentId: "pi_mcp_scheduler_123_corrected",
      paymentLink: "https://pay.stripe.com/test_mcp_scheduler",
      paymentNotes: "Corrected by agent from scheduler checkout settlement.",
      paymentSourceType: "project_source",
      paymentSourceId: sourceCall.result.structuredContent.projectSource.id,
      inviteeAnswers: [
        {
  ...
  assert.equal(contextInvoice.balanceCents, 999);
  assert.equal(contextInvoice.clientPayableBalanceCents, 999);
  ...
  const contextAgentInvoice = contextCall.result.structuredContent.projectContext.invoices.find((invoice: { invoiceNumber: string }) => invoice.invoiceNumber === "INV-MCP");
  assert.equal(contextAgentInvoice.sourceType, "project_source");
  assert.equal(contextAgentInvoice.sourceId, sourceCall.result.structuredContent.projectSource.id);
  assert.equal(contextAgentInvoice.totalCents, 1200000);
  assert.equal(contextAgentInvoice.clientPayableCents, 1234830);
  assert.equal(contextAgentInvoice.balanceCents, 1234830);
  assert.equal(contextAgentInvoice.clientPayableBalanceCents, 1234830);
  ...
  assert.deepEqual(contextInvoice.paymentSummary, {
    ...
    paidCents: 99000,
    grossCollectedCents: 101931,
    clientFeeCents: 2931,
    processingFeeCents: 2931,
    netDepositCents: 99000,
    openCents: 0,
  });
  assert.deepEqual(contextCall.result.structuredContent.projectContext.financialSummary, {
    scheduledCents: 1324000,
    paidCents: 123000,
    grossCollectedCents: 126657,
    clientFeeCents: 3657,
    processingFeeCents: 3657,
    netDepositCents: 123000,
    openCents: 1200000,
    clientPayableOpenCents: 1235829,
    expenseCents: 13500,
    paidExpenseCents: 13500,
    openPayableCents: 0,
    taxDeductibleExpenseCents: 13500,
    nonDeductibleExpenseCents: 0,
    collectedProfitCents: 109500,
    projectedProfitCents: 1310500,
    invoiceCount: 2,
    invoicePaymentCount: 5,
  ...
  assert.equal(financeReportCall.result.structuredContent.financeReport.paymentLedger.totals.paidCents, 123000);
  ...
  assert.deepEqual(financeReportCall.result.structuredContent.financeReport.projectFinancials, [
    {
      ...
      collectedProfitCents: 109500,
      projectedProfitCents: 110500,
    },
  ]);
  assert.equal(
    financeReportCall.result.structuredContent.financeReport.paymentLedger.rows.find((row: { paymentId: string }) => row.paymentId === "payment-2").paymentSourceId,
    sourceCall.result.structuredContent.projectSource.id,
  );
  assert.equal(
    financeReportCall.result.structuredContent.financeReport.paymentLedger.rows.find((row: { paymentId: string }) => row.paymentId === "payment-2").clientPayableOpenCents,
    0,
  );
  assert.equal(
