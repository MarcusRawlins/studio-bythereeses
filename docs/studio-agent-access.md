...
| `UNSUBSCRIBE_SECRET` *(Phase 8c — sequences)* | app Worker secret | HMAC key for the stateless one-click unsubscribe token (`src/lib/unsubscribe-token.ts`), verified constant-time at `/api/email/unsubscribe`. **Fail-closed:** verify returns null (route `400`s) if unset — no suppression on an unconfigured secret. Never log. | Suspected exposure (rotating invalidates outstanding unsubscribe links; clients can re-unsubscribe) |
| `CRON_SECRET` *(shared — reminders + Phase 8c sequences)* | app Worker secret **+** `reese-sequence-runner` worker (and `reese-scheduler-reminders`) | Bearer for `POST /api/cron/sequences` (and `/api/cron/scheduler-reminders`), constant-time compared. **Fail-closed 503 when unset**, 401 on mismatch. Must match on both the origin and the cron worker. | Suspected exposure |
| `SEQUENCES_ENABLED` *(Phase 8c)* | app Worker var | Master flag for the sequence runner (`src/lib/sequences.ts`). **Default OFF** (`"1"` to enable). Unset ⇒ `/api/cron/sequences` returns `{skipped:"flag_off"}` and `runDueSequences` no-ops. | n/a (flag) |
| `SEQUENCES_DUNNING` / `SEQUENCES_PREEVENT` / `SEQUENCES_REVIEW` *(Phase 8c)* | app Worker var | Per-sequence enable. **Default OFF.** A sequence off ⇒ its enrollments are neither created nor advanced. | n/a (flag) |
| `SEQUENCES_DUNNING_AUTOSEND` / `SEQUENCES_PREEVENT_AUTOSEND` *(Phase 8c)* | app Worker var | Per-sequence, **EMAIL-ONLY** auto-send opt-in. **Default unset ⇒ DRAFT** for approval. Only when `"1"` (and the step is auto-send-eligible and channel email) does a step auto-send. Review has no auto-send flag (draft-only by design). SMS NEVER auto-sends. | n/a (flag) |
| `SEQUENCES_MAX_PER_CLIENT_PER_WEEK` / `SEQUENCES_MAX_ATTEMPTS` *(Phase 8c)* | app Worker var | Rolling per-client comms cap (default `3`) and provably-not-sent retry bound (default `2`). | n/a (tunable) |
| `SEQUENCES_ENDPOINT` *(Phase 8c)* | `reese-sequence-runner` worker var | Split worker → app. **workers.dev ORIGIN, not the proxy host:** `https://reese-photography-crm.solitary-flower-c3ab.workers.dev/api/cron/sequences`. Pointing it at `studio.bythereeses.com` login-walls it (silent drop); the worker throws on any redirect/non-2xx. | On origin URL change |

## Phase 8c — Automated Sequences Enable Runbook (Tyler)

Built **dark**: `SEQUENCES_ENABLED` off, every per-sequence flag off, every auto-send flag off, and the cron trigger not registered. 100% draft-for-approval until deliberately opted in. Enablement flips are NOT autonomous.

**Rollout order:** (1) apply migration `0088` (additive + dark — new `sequence_enrollments`, `sequence_sends`, `email_suppressions` tables; NOT deploy-order-critical, apply anytime via the idempotent direct `d1 execute --file` pattern). (2) Deploy the app Worker (`sequences.ts`, `/api/cron/sequences`, `/api/email/unsubscribe`, `origin-guard` `PUBLIC_API_PREFIXES += /api/cron/sequences`) with all `SEQUENCES_*` flags unset. (3) Deploy the Pages-proxy with the `/api/email/unsubscribe` classifier (`isStudioPublicPath` + admin-proof exemption + `publicMutation` rate-limit + drift test) — the cron path needs **no** proxy edit (it reaches the origin directly).

**Enable steps:** 1. Confirm `RESEND_API_KEY`/`RESEND_FROM_EMAIL` set. Set `UNSUBSCRIBE_SECRET` (app Worker) and `CRON_SECRET` on **both** the app Worker and `reese-sequence-runner` (must match). 2. `wrangler deploy -c wrangler.sequence-runner.jsonc`; confirm `SEQUENCES_ENDPOINT` is the workers.dev origin. 3. Set `SEQUENCES_ENABLED=1` + **one** sequence flag (e.g. `SEQUENCES_REVIEW=1`) → watch drafts for a cycle; auto-send flags stay OFF. 4. Only after drafts read correctly, optionally flip a single `SEQUENCES_<NAME>_AUTOSEND=1` for an email sequence. 5. Test: enroll a fixture, POST the cron with the bearer, confirm a draft/ledger row + a real unsubscribe round-trip.

**Rollback (instant, non-destructive):** unset/`0` `SEQUENCES_ENABLED` ⇒ runner no-ops; delete the cron trigger (or the `reese-sequence-runner` worker) ⇒ the evaluator never runs. `0088` tables are additive/inert when flags are off; no down-migration. **Do NOT drop `email_suppressions`** — a client's unsubscribe must survive a feature toggle (same rule as `sms_suppressions`).
...
- `studio_get_daily_brief`: reads the rule-based daily brief (priority leads/stuck items/overdue payments, upcoming shoots/calls, and — when `PROJECT_PROGRESS_TIMELINE=1` — overdue project milestones). READ-ONLY — the agent composes the prose narrative from this JSON; the tool never sends or writes anything. The narration itself is stored by a separate REST-only write, `POST /api/agent/daily-brief-narrative` (double-guarded exactly like `/api/agent/health`; not an MCP tool — see `docs/specs/phase-18-ai-daily-brief.md` §3.3).
...
- `studio_draft_email` *(Phase 14)*: drafts an email for Tyler to review and send. **Draft-only — never sends.** Always writes a `channel: "email"`, `status: "draft"`, `direction: "outbound"`, `createdBy: "agent"` communication row regardless of any `status`/`channel`/`createdBy` supplied, so an agent can never mint a "sent" email or trigger a Resend delivery. The real enforcement is the same table-level boundary, now widened to email: `createProjectCommunicationFromAgent`/`updateProjectCommunicationFromAgent` clamp any agent-authored `email` (or re-channeled-to-email) row to `draft`, so neither the generic `studio_create_communication`/`studio_update_communication` tools nor a re-channel-in-one-patch can bypass it. Only Tyler's admin "send approved email" action (`sendApprovedProjectEmail`, content-bound by a sha256 of the reviewed subject **+** body **+** resolved recipient, so a post-review recipient swap is refused) actually sends, and only when `EMAIL_SENDING_ENABLED=1` and the recipient is not suppressed. Inbound client replies attach to the existing project thread only via a signed reply token (`src/lib/project-reply-token.ts`), never a spoofable `From`.
