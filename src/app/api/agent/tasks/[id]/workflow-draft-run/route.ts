...
import { runStudioWorkflowDraftTask } from "@/lib/studio-mcp";
...
export async function POST(
  ...
  try {
    ...
    if (!workflowDraftRun) {
      return NextResponse.json({ workflowDraftRun: null }, { status: 404 });
    }

    return NextResponse.json({ workflowDraftRun });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Workflow draft run failed." },
      { status: 400 },
    );
  }
}
