...
| 9b | Refund INITIATION (admin-triggered Stripe refund — MOVES money) | ✅ built + deployed DARK 2026-07-06 (Worker `c52f2dc6`, rollback `3fa37e5a`, migration 0091), `REFUND_INITIATION_ENABLED` **OFF** — no money can move; routes admin-only + origin-guarded (execute 404s on workers.dev). Spec Fable-gated ×4, code Fable-gated ×2 (a concurrent-double-POST BLOCKER caught + fixed via claim-token CAS). Admin refund UI + reconciliation wiring built + deployed dark 2026-07-06 (Worker `5c7d3047`), Fable-approved. 🅣 **ENABLE is Tyler's** — remaining: auth-armed + `charge.refunded` webhook + `FINANCE_REFUND_RECORDING=enforce` preconditions, 2 policy confirmations (retainer rule, service-ceiling), then flag flip + first-refund watch. |
