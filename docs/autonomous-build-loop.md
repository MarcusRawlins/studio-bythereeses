...
| 9b | Refund INITIATION (admin-triggered Stripe refund — MOVES money) | ✅ built + deployed DARK 2026-07-06 (Worker `c52f2dc6`, rollback `3fa37e5a`, migration 0091), `REFUND_INITIATION_ENABLED` **OFF** — no money can move; routes admin-only + origin-guarded (execute 404s on workers.dev). Spec Fable-gated ×4, code Fable-gated ×2 (a concurrent-double-POST BLOCKER caught + fixed via claim-token CAS). 🅣 **ENABLE is Tyler's** — needs the pre-enable checklist (admin refund UI, wire reconciliation, auth-armed + webhook + `enforce` preconditions, 2 policy confirmations) then flag flip + first-refund watch. |
